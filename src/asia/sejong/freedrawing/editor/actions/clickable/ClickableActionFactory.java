...
public class ClickableActionFactory {
	...
	public static ColorPickAction createColorPickAction(ContextManager cm, EditDomain domain, IWorkbenchPart part) {
		return (ColorPickAction)AbstractClickableAction.build(ColorPickAction.class, "칼라선택", cm.getImageManager().getColorPickImageDescriptor(), domain, part);
	}

	public static FontPickAction createFontPickAction(ContextManager cm, DefaultEditDomain domain, IWorkbenchPart part) {
		return (FontPickAction)AbstractClickableAction.build(FontPickAction.class, "칼라선택", cm.getImageManager().getFontPickImageDescriptor(), domain, part);
	}
}
