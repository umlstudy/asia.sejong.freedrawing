...
public class Rotation7PushRestore {
...
//				System.out.printf("ocp.x ? %d, ocp.y %d\n", ocp.x, ocp.y);
				...
				g.pushState();
				//g.clipRect(new org.eclipse.draw2d.geometry.Rectangle(rect.x,rect.y, rect.width, rect.height));
				//g.setClip(new org.eclipse.draw2d.geometry.Rectangle(rect.x,rect.y, rect.width, rect.height));
				g.rotate(10f);
				
				double r = degree * 3.141592 / 10.0;
			    float cos45 = (float)Math.cos(r);
			    float sin45 = (float)Math.sin(r);
				...
				//g.fillRectangle(0,0, rect.width, rect.height);
